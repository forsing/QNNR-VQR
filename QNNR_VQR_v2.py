...
NN = df.shape[0]  # v2: kao 4512 u polaznom — svi redovi ovog CSV
...
# Draw circuits → PNG (v2: bez interaktivnog prozora)
_out_circ = Path(__file__).resolve().parent / "QNNR_VQR_v2_out"
_out_circ.mkdir(parents=True, exist_ok=True)
_fm = feat_map.decompose().draw("mpl")
_fm.savefig(_out_circ / "feat_map.png", dpi=120, bbox_inches="tight")
plt.close("all")
_an = ansatz.decompose().draw("mpl")
_an.savefig(_out_circ / "ansatz.png", dpi=120, bbox_inches="tight")
plt.close("all")
print(f"[v2] Sačuvano: {_out_circ / 'feat_map.png'}, {_out_circ / 'ansatz.png'}")
